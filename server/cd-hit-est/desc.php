...
$opt="-d 0 -n 10 -l 11 -r 1 -p 1 -G 0 -c 0.95 -aS 0.8";
$opt_desc="
    ...
    -r  1 or 0, default 0, by default only +/+ strand alignment
        if set to 1, do both +/+ & +/- alignments    
";

$cmd='./client_submit_job.pl input.fasta cd-hit-est output.zip "you@example.com" "-d 0 -n 10 -l 11 -r 1 -p 1 -G 0 -c 0.95 -aS 0.8"

Notes:
input.fasta: DNA input FASTA file (required)
cd-hit-est: program name (required)
output.zip: output file (required)
"you@example.com": your email (optional)
"-d 0 -n 10 -l 11 -r 1 -p 1 -G 0 -c 0.95 -aS 0.8": parameters for cd-hit-est program (optional)
...
./client_submit_job.pl input.fasta cd-hit-est output.zip "" "-d 0 -n 10 -l 11 -r 1 -p 1 -G 0 -c 0.95 -aS 0.8"
For the second example, paramters are empty:
./client_submit_job.pl input.fasta cd-hit-est output.zip "you@example.com"
For third example, both email and parameters are empty(using default parameters):
./client_submit_job.pl input.fasta cd-hit-est output.zip
';
?>
