...
$prog_overview="<h3> <center>function annotation (KOG)</center> </h3>
This program performs function annotation by using RPSBLAST program on KOG database (eukaryotic proteins). 
<br />
<h5>Inputs:</h5>
...
";
$cmd='./client_submit_job.pl input.fasta kog output.zip "you@example.com" "-evalue 0.001"

Notes:
input.fasta: protein input FASTA file (required)
kog: program name (required)
...
./client_submit_job.pl input.fasta kog output.zip "" "-evalue 0.001"
For the second example, paramters are empty:
./client_submit_job.pl input.fasta kog output.zip "you@example.com"
For the third example, both email and parameters are empty(using default parameters):
./client_submit_job.pl input.fasta kog output.zip
';
