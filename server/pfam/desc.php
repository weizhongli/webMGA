...
$prog_overview="<h3> <center>function annotation (PFAM)</center> </h3>
...
<br />
2. \"The Pfam protein families database\", R. D. Finn, et al. Nucleic Acids Rese
arch (2010) 38: D211-D222.
";

$opt="-E 0.001";
$opt_desc="-E e-value cutoff for prediction 
";
$cmd='./client_submit_job.pl input.fasta pfam output.zip "you@example.com" "-E 0.001"

Notes:
input.fasta: protein input FASTA file (required)
pfam: program name (required)
...
./client_submit_job.pl input.fasta pfam output.zip "" "-E 0.001"
For the second example, paramters are empty:
./client_submit_job.pl input.fasta pfam output.zip "you@example.com"
For the third example, both email and parameters are empty(using default parameters):
./client_submit_job.pl input.fasta pfam output.zip
';
?>
