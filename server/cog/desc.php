...
$prog_overview="<h3> <center>function annotation (COG)</center> </h3>
...
$cmd='./client_submit_job.pl input.fasta cog output.zip "you@example.com" "-evalue 0.001"

Notes:
input.fasta: protein input FASTA file (required)
cog: program name (required)
...
./client_submit_job.pl input.fasta cog output.zip "" "-evalue 0.001"
For the second example, paramters are empty:
./client_submit_job.pl input.fasta cog output.zip "you@example.com"
For the third example, both email and parameters are empty(using default parameters):
./client_submit_job.pl input.fasta cog output.zip
';
?>
